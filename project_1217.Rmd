...
#litter_bin is continuous so binning it in order to asseess interaction
df$litter_bin = cut(
...
As population increases, ZIP codes with food scrap drop-off sites experience a faster increase in rat inspections than ZIP codes without drop-off sites.

ZIP codes containing DSNY garages have substantially higher median population than those without garages and span a narrower portion of the population distribution. This limited overlap results in unstable estimation of population-dependent effects within the garage group, suggesting that apparent slope differences reflect data structure rather than a meaningful interaction. In contrast, food scrap drop-off sites are observed across a wider range of population levels, allowing for more reliable assessment of interaction effects with population
...
bc = boxcox(box_cox_base_model, lambda = seq(-1, 1, by = 0.05))
...
The histogram of studentized residuals is approximately symmetric and centered near zero, with most observations falling within ±2. Mild right-tail behavior remains, consistent with the presence of a small number of high-inspection ZIP codes, but no severe departures from normality are observed.

A normal Q–Q plot of the studentized residuals shows strong agreement with normality across the central quantiles, with only mild deviations in the upper tail. This behavior is consistent with the log-transformed count nature of the response and does not indicate a substantial violation of model assumptions

Shapiro-Wilk Normality Test---> Null Hypothesis is Residuals are Normal. P-value = 0.7877 ==> no evidence against normality. W ~~ 1 ==> No evidence against nromality 